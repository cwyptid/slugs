...
function initializeGarden() {
  ...
  gardenState.tonyActionIdleSprite = new Sprite(
...
function drawGardenMode(currentTime) {
  ...
  for (let plant of currentSectionPlants) {
    ...
    let frame = plant.watered
      ? plant.wateredSprite.getCurrentFrame()
      : plant.drySprite.getCurrentFrame();
    if (frame) {
  ...
  if (currentSection === 2) {
    ...
    let shellSprite = allPlantsWatered
  ...
  if (sectionTransition.fading) {
    ...
    const fadeAlpha =
      progress < 0.5 ? progress * 2 * 255 : (1 - progress) * 2 * 255;
...
function drawNavigationArrows(currentTime) {
  ...
  // Left arrow
  if (
    (gameMode === "garden" || gameMode === "intro") &&
    currentSection > 1 &&
...
function checkIfHoveringPlant() {
  ...
  if (currentSection === 3) {
    if (
      mouseX > gardenState.emptyPlot.x &&
      mouseX < gardenState.emptyPlot.x + gardenState.emptyPlot.width &&
      mouseY > gardenState.emptyPlot.y &&
      mouseY < gardenState.emptyPlot.y + gardenState.emptyPlot.height
    ) {
  ...
  if (currentSection === 2) {
    if (
      mouseX > gardenState.shell.x &&
      mouseX < gardenState.shell.x + gardenState.shell.width &&
      mouseY > gardenState.shell.y &&
      mouseY < gardenState.shell.y + gardenState.shell.height
    ) {
      isHoveringInteractiveArea = true;
      return;
    }
