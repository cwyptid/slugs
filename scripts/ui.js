...
function initializeNameInputButtons() {
  ...
  const totalLetterButtonsWidth =
    LETTERS_PER_ROW * BUTTON_SIZE + (LETTERS_PER_ROW - 1) * BUTTON_SPACING;
  ...
  const actionButtonY =
    BUTTON_ROWS_START_Y + LETTER_ROWS * (BUTTON_SIZE + BUTTON_SPACING) + 40;
...
function drawNameInputScreen() {
  ...
  for (let i = 0; i < nameInputButtons.length; i++) {
    ...
    const bounceAmount =
      (isHovered && selectedButtonIndex === -1) || isSelected ? -12 : 0;
...
function drawGardenMode(currentTime) {
  ...
  if (
    ...
    let leftArrowHovered =
      mouseX > arrowLeftX &&
      mouseX < arrowLeftX + arrowSize &&
      mouseY > arrowBaseY - bobAmount &&
      mouseY < arrowBaseY + arrowSize + bobAmount;
  ...
  if (
    ...
    let rightArrowHovered =
      mouseX > arrowRightX &&
      mouseX < arrowRightX + arrowSize &&
      mouseY > arrowBaseY - bobAmount &&
      mouseY < arrowBaseY + arrowSize + bobAmount;
  ...
  if (sectionTransition.fading) {
    const elapsed = currentTime - sectionTransition.startTime;
    const progress = Math.min(elapsed / sectionTransition.duration, 1);
    ...
    if (
      progress >= 0.5 &&
      currentSection !== sectionTransition.nextSectionTarget
    ) {
      currentSection = sectionTransition.nextSectionTarget;
    }
    ...
    if (progress >= 1) {
      sectionTransition.fading = false;
    }
  }
}
...
function drawVNMode(currentTime) {
  ...
  const typewriterDelayDuration = 0;
  ...
  if (
    ...
    if (!typewriterFinished) {
      console.log(
        `[DRAW] Typewriter JUST finished. Scene: ${currentScene}, charsToShow: ${charsToShow}, totalChars: ${cachedDialogueOnly.length}`,
      );
...
function drawStoryTransitionScreen(currentTime) {
  ...
  if (charsToShow >= allChars) {
    ...
    if (elapsedTime >= storyTransition.duration) {
      ...
      introStartTime = currentTime + fades.toIntro.duration;
      introPhase = "none";
...
function startSectionTransition(targetSection) {
  ...
  sectionTransition.fading = true;
  sectionTransition.nextSectionTarget = targetSection;
  sectionTransition.startTime = millis();
}
...
function keyPressed() {
  ...
  if (DEBUG_CUTSCENE && key === "`") {
    ...
    fades.toCutscene.fading = false;
    fades.cutsceneEnd.fading = false;
...
function mousePressed() {
  ...
  if (gameMode === "storyTransition") {
    ...
    fades.toIntro.fading = true;
    fades.toIntro.startTime = millis();
    introStartTime = millis() + fades.toIntro.duration;
    introPhase = "none";
    ...
    for (let plant of currentSectionPlants) {
      if (
        ...
        if (!plant.watered || DEBUG_MODE) {
          ...
          flavor = wateredFlavor;
          flavorText = flavor[plant.id];
          flavorTextTimer = millis() + FLAVOR_TEXT_DURATION;
        }
        return false;
      }
  ...
  if (gameMode === "vn" || gameMode === "cutscene") {
    ...
    if (currentTime - lastClickTime < CLICK_COOLDOWN) {
      console.log(
        `[CLICK DEBOUNCED] Too soon after last click (${currentTime - lastClickTime}ms since last)`,
      );
      return false;
    }
    ...
    console.log(
      `[CLICK ${clickId}] START - Scene: ${currentScene}, typewriterFinished: ${typewriterFinished}`,
    );
...
function mouseMoved() {
  ...
  if (gameMode === "rain_ending") {
    if (showESCOverlay) {
      ...
      const _bW = 254 * canvasScale,
        _bH = 85 * canvasScale,
        _bY = height * 0.55,
        _gap = 40 * canvasScale;
      const _lX = width / 2 - _bW - _gap / 2,
        _rX = width / 2 + _gap / 2;
      const _nowH =
        mouseX > _lX && mouseX < _lX + _bW && mouseY > _bY && mouseY < _bY + _bH
          ? "stay"
          : mouseX > _rX &&
              mouseX < _rX + _bW &&
              mouseY > _bY &&
            ...
            ? "return"
            : null;
      if (_nowH !== lastESCHoveredButton) {
        lastESCHoveredButton = _nowH;
        if (_nowH && assets.hoverSound) {
          assets.hoverSound.setVolume(0.2);
          assets.hoverSound.play();
        }
      }
  ...
  if (!isCurrentlyHoveringPlant && !isHoveringInteractiveArea) {
    if (currentSection === 3) {
      const area = gardenState.emptyPlot;
      isHoveringInteractiveArea =
        mouseX > area.x &&
      ...
      currentSection === 2 &&
      (!gardenState.shell.visited || DEBUG_MODE)
    ) {
      const area = gardenState.shell;
      isHoveringInteractiveArea =
        mouseX > area.x &&
